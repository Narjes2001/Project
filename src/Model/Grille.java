...
public class Grille extends Observable {
    ...
    public Grille(ArrayList<Villageois> villageois){
        ...
        this.hotelVille = new Batiment(positionHotelVille, "HotelVille", 12, 12);
        ...
        this.cabanes.add(new Batiment(positioncabane, "Cabane", 8, 8));
    ...
    public Point EmplacementCible(int x, int y){
    ...
    public void addMonstres(){
        ...
        Point cible = this.EmplacementCible(res.x, res.y);
        this.monstres.add(new Monstre(res.x, res.y, cible));
    ...
    public void addCabanes() {
        // Verification pour ne pas avoir plus de 5 cabanes
        if(this.cabanes.size() < 5) {
            // Generation de la position aleatoire pour la cabane
            Point positioncabane = genererPoint();
            // Ajout de la cabane a la liste des cabanes
            this.cabanes.add(new Batiment(positioncabane, "Cabane", 8, 8));
            // Verification que la cabane ne sort pas de la grille
            while (positioncabane.x + this.cabanes.get(0).getLargeur() >= Grille.LARGEUR || positioncabane.y + this.cabanes.get(0).getLargeur() >= Grille.HAUTEUR) {
                positioncabane = genererPoint();
            }
            // Mise a jour de la position de la cabane
            this.cabanes.get(0).setPosition(positioncabane);
            // Boucle pour marquer les cases occupees par la cabane
            for (int i = positioncabane.x; i < positioncabane.x + this.cabanes.get(0).getLargeur(); i++) {
                for (int j = positioncabane.y; j < positioncabane.y + this.cabanes.get(0).getHauteur(); j++) {
                    // Mise a jour des cases avec le batiment
                    this.cases[i][j].setBatiment(this.cabanes.get(0));
                    // Mise a jour de l'etat de la case (occupee)
                    this.cases[i][j].setOccupee(true);
                }
            }
            // Calcul du nombre de villageois a ajouter
            int nbAddVillageois = this.cabanes.size() * 2 - this.cabanes.size();
            // Stockage du nombre initial de cabanes
            int nbInit = this.cabanes.size();
            // Boucle pour ajouter des villageois
            for (int i = 0; i < nbAddVillageois; i++) {
                // Generation de la position aleatoire pour le villageois
                Point positionVillageois = genererPoint();
                // Ajout du villageois a la liste des villageois
                this.villageois.add(new Villageois(positionVillageois.x, positionVillageois.y));
            }
            // Boucle pour mettre a jour les cases occupees par les villageois
            for (int i = nbInit - 1; i < this.villageois.size(); i++) {
                // Recuperation de la position du villageois
                int x = this.villageois.get(i).getPosition().x;
                int y = this.villageois.get(i).getPosition().y;
                // Mise a jour des cases avec le villageois
                this.cases[x][y].setVillageois(this.villageois.get(i));
            }
        }
    }
